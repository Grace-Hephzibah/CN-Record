...
public class Exp13_Client
{
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            ds.receive(drp); inMsg = new String(drp.getData(), 0, drp.getLength());
            ...
            else{ System.out.println("Connection refused by vpn "+Arrays.toString(vpnDetails));
                System.out.println();
            }
        }
        catch(Exception e){ e.printStackTrace(); }
        if (vpnConnect==true) {
            System.out.print("Enter Initial Server IP : "); serverIP = sc.next();
            System.out.print("Enter Initial server Port : "); serverPort = sc.nextInt();
            sc.nextLine(); System.out.println(); server = new String[]{serverIP, "" + serverPort,""};
            try{
                DatagramSocket ds=new DatagramSocket(); DatagramPacket dsp,drp;
                do{
                    System.out.print("Enter something : "); server[2] = sc.nextLine();
                    outMsg = encode(server); outData = outMsg.getBytes();
                    dsp = new DatagramPacket(outData, outData.length, InetAddress.getByName(vpnDetails[0]),
                            Integer.parseInt(vpnDetails[1])); ds.send(dsp);
                    if (server[2].equalsIgnoreCase("bye")) { break; }
                    inData = new byte[1024]; drp = new DatagramPacket(inData,inData.length);
                    ds.receive(drp); inMsg = new String(drp.getData(), 0, drp.getLength());
                    vpnResponse = decode(inMsg); System.out.println("Server msg : " + vpnResponse[2]);
